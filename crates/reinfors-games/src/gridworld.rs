...
//! `chance_outcomes` is the default (empty).
...
impl Game for GridWorld {
    ...
    fn step_env(
        ...
        state: &GridState,
        actions: &[usize],
        _rng: &mut dyn reinfors_core::Rng,
    ) -> Transition<GridState> {
        self.step(state, actions) // deterministic: the realized transition is the believed one
    }
}
...
mod tests {
    ...
    fn search_finds_the_step_onto_the_goal() {
        ...
        let results = search_many(&w, &cfg(), vec![(start, 0)], false, zero_infer);
    ...
    fn engine_rolls_out_a_single_agent_game() {
        ...
        let planner = SelectiveTreeStrap::new(cfg(), 0.3, false);
        ...
        let mut engine = Engine::new(world(), planner, params);

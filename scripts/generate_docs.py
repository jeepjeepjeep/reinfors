...
def games_page() -> str:
    ...
    return (
        ...
        + "- Snapshots restore the exact saved state and always resume the same trajectory, "
        + "but that trajectory departs from a never-snapshotted run at float rounding order "
        + "(the physics solver's contact machinery is rebuilt rather than deserialized, which "
        + "is what keeps hostile snapshot bytes safe).\n"

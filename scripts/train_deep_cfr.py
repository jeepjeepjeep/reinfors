...
def holdem_eval(policy_infer: InferFn, hands: int, seed: int) -> dict[str, float]:
    ...
    for opponent in ("random", "call"):
        ...
        for hand in range(hands):
            env = rf.Env(rf.games.TexasHoldem(num_players=2), rf.Reward(), seed=rng.randrange(2**31))
            env.reset()
            seat = hand % 2
            while not env.done():
...
def main() -> None:
    ...
    handle = {
        "kuhn_poker": lambda: rf.games.KuhnPoker(),
        "leduc_poker": lambda: rf.games.LeducPoker(),
        "texas_holdem": lambda: rf.games.TexasHoldem(num_players=2),
    }[args.game]()
    ...
    for it in range(1, args.iterations + 1):
        ...
        if args.eval_every and it % args.eval_every == 0:
            ...
            line = (
                f"iter {it:4d}  wall {wall:6.1f}s  "
                f"adv buffers {advantage_buffers[0].size}/{advantage_buffers[1].size}  "

...
def test_connect4_played_to_a_win() -> None:
    ...
    last: list[tuple[int, object]] = []
    for agent, col in moves:
        assert env.active_agents() == [agent]  # sequential: one mover per tick
        last = env.step({agent: col})  # the tick's (agent, event) trace (Env holds no reward)
    assert env.done()
    assert last == [(0, "win"), (1, "loss")]  # the terminal trace carries the outcome: P0 wins
...
def test_snake_steps_both_agents_simultaneously() -> None:
    ...
    trace = env.step({0: 1, 1: 1})  # both move forward; (agent, event-dict) trace entries
    assert [agent for agent, _ in trace] == [0, 1]
    assert all("died" in e for _, e in trace)
